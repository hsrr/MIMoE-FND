...
class Vimoe_V2(nn.Module):
    def __init__(
        ...
        self.m_i_projection = nn.Sequential(
            nn.Linear(clip_embed_dim, clip_hidden_dim),
            nn.Linear(clip_hidden_dim, 64),
            nn.SiLU(),
        )
        ...
        self.final_fusing_experts = nn.ModuleList([_expert() for i in range(4)])
